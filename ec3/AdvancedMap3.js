 ...
 */
ADMap.Event={
	FRONT:0,//常量，表示监听器数组开头位置
	observers:false,//被观察对象
	listeners:{},//被观察对象监听器
	listenerCount:{},//被观察对象名称及其监听器数量
	/**
	 * addListener 注册事件
...
* @private
*/
ADMap.BaseLayer.prototype._bindEvent = function () {
    ...
    //occured before the zoom event is triggered.
...
* @private
*/
ADMap.BaseLayer.prototype._moveHandler = function (type) {
    var _this = this;
 ...
 * @return {void}
 */
ADMap.BaseLayer.prototype._unbindEvent = function () {
...
ADMap.BaseLayer.prototype.refresh = function (isAnimation) {
    ...
    if (this._ec) {
        ...
        if(this._type==ADMap.Type.MIGRATION){
            if(!isAnimation){
...
/*
 * 使用echart的markline和markpoint中的large配置项，停止动画渲染
 * @param  {[type]} option [description]
 * @return {[type]}        [description]
 * @private
 */
ADMap.BaseLayer.prototype._pauseLineAnimation=function(option){
    var s=option.series;
    for(var i=0;i<s.length;i++){
    	s[i].animation=false;

        if(s[i].effect){
            s[i].effect.show=false;
        }
    }
}
/*
 * 使用echart的markline和markpoint中的large配置项，开启动画渲染
 * @param  {[type]} option [description]
 * @return {[type]}        [description]
 * @private
 */
ADMap.BaseLayer.prototype._startLineAnimation=function(option){
    var s=option.series;
    for(var i=0;i<s.length;i++){
        s[i].animation=true;

        if(s[i].effect){
            s[i].effect.show=true;
        }
    }
}

/**
 * MigrationLayer 迁徙图图层
 * @param {object} option admap配置项
 */
ADMap.MigrationLayer=function(option){
    ...
    this._map.once('postrender',this._render,this);
}
...
ADMap.MigrationLayer.prototype._render=function(){
	this.initEChart();
    var series=new ADMap.Source({
		data:this._source,
		map:this._map
	}).migration();
	var opt=new ADMap.Style().migration(series);
    this.setOption(opt,true);
}


/**
 * Source 资源类，提供迁徙图，热图等的标准数据格式
 * @param {object} option 包含地图对象和数据对象
 */
...
/**
 * migrationFormatter 将数据格式化为迁徙图数据
 * @param  {object} data 原始数据
 * @return {object}      标准迁徙图数据
 */
...
/**
 * Style 样式类，提供迁徙图，热图等的默认样式
 * @param {} option
 */
ADMap.Style=function(option){
	this._option=option;
}
/**
 * migration 提供迁徙图样式
 * @param  {object} series echart的数据系列
 * @return {object}        标准echart配置项
 */
ADMap.Style.prototype.migration=function(series){
	return{
        title: {
